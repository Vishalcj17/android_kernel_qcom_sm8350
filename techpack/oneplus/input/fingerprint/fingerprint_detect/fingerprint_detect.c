...
int get_fp_version(void)
{
	if (0 == fp_version) {
		pr_info("%s get_fp_version failed.\n", __func__);
	} else {
		pr_info("%s get_fp_version :%d.\n", __func__, fp_version);
	}
	return fp_version;
}
EXPORT_SYMBOL(get_fp_version);
/*
				...
				const char *buffer, size_t count)
{
	...
	if (count < 32)
	{
		kstrtoint(buffer, 10, &(fp_detect->sensor_version));
	} else {
