...
#define OP_ATTR(_name, _mode, _show, _store)                                   \
	struct kobj_attribute op_attr_##_name =                                \
...
uint8_t get_secureboot_fuse_status(void)
{
	void __iomem *oem_config_base;
	uint8_t secure_oem_config = 0;
...
static enum UFS_SIZE ufs_get_size_by_pn(char *pn)
{
	...
	while (ufs_64gb_pn_list[i]) {
		if (!strncmp(ufs_64gb_pn_list[i], pn,
			     strlen(ufs_64gb_pn_list[i])))
	...
	while (ufs_128gb_pn_list[i]) {
		if (!strncmp(ufs_128gb_pn_list[i], pn,
			     strlen(ufs_128gb_pn_list[i])))
	...
	while (ufs_256gb_pn_list[i]) {
		if (!strncmp(ufs_256gb_pn_list[i], pn,
			     strlen(ufs_256gb_pn_list[i])))
	...
	while (ufs_512gb_pn_list[i]) {
		if (!strncmp(ufs_512gb_pn_list[i], pn,
			     strlen(ufs_512gb_pn_list[i])))
...
static struct attribute *project_info_sysfs_entries[] = {
	&op_attr_project_name.attr,   &op_attr_hw_id.attr,
	&op_attr_rf_id_v1.attr,	      &op_attr_rf_id_v2.attr,
	&op_attr_rf_id_v3.attr,	      &op_attr_ddr_manufacture_info.attr,
	&op_attr_ddr_row.attr,	      &op_attr_ddr_column.attr,
	&op_attr_ddr_fw_version.attr, &op_attr_ddr_reserve_info.attr,
	&op_attr_secboot_status.attr, &op_attr_platform_id.attr,
	&op_attr_serialno.attr,	      &op_attr_aboard_id.attr,
	&op_attr_add_component.attr,  NULL,
};
...
static int project_info_init_sysfs(void)
{
	...
	component_info =
		kobject_create_and_add("component_info", project_info_kobj);
...
struct ddr_manufacture ddr_manufacture_list[] = {
	{ 1, "Samsung " }, { 2, "Qimonda " }, { 3, "Elpida " },
	{ 4, "Etpon " },   { 5, "Nanya " },   { 6, "Hynix " },
	{ 7, "Mosel " },   { 8, "Winbond " }, { 9, "Esmt " },
	{ 255, "Micron" }, { 0, "Unknown" },
};
...
struct cpu_list cpu_list_msm[] = {
	{ 321, "SDM845 " }, { 339, "SM8150 " }, { 356, "SM8250 " },
	{ 415, "SM8350 " }, { 0, "Unknown" },
};
	...
	if (IS_ERR_OR_NULL(project_info_desc)) {
		...
		for (i = 0; i < length; i++) {
			if (cpu_list_msm[i].id ==
			    project_info_desc->platform_id) {
				snprintf(cpu_type, sizeof(cpu_type), "%s",
					 cpu_list_msm[i].name);
				break;
			}
		}
...
static char mainboard_manufacture[8] = {
	'O', 'N', 'E', 'P', 'L', 'U', 'S', '\0'
};
...
static int init_project_info(void)
{
	...
	p = &main_board_info_check[ARRAY_SIZE(main_board_info_check) - 1]
		     .version_name[0];

	for (i = 0; i < ARRAY_SIZE(main_board_info_check); i++) {
		if (project_info_desc->prj_version ==
			    main_board_info_check[i].prj_version &&
		    project_info_desc->hw_version ==
			    main_board_info_check[i].hw_version &&
		    (project_info_desc->rf_v1 ==
			     main_board_info_check[i].rf_version ||
	...
	pr_err("board info: %s\n", mainboard_version);
	push_component_info(MAINBOARD, mainboard_version,
			    mainboard_manufacture);

	snprintf(rf_version, sizeof(rf_version), " %d",
		 project_info_desc->rf_v1);
	...
	snprintf(ddr_manufacture_and_fw_verion,
		 sizeof(ddr_manufacture_and_fw_verion), "%s%s %u.%u",
		 ddr_manufacture,
		 project_info_desc->ddr_reserve_info == 0x05 ?
			       "20nm" :
	...
	push_component_info(LCD, op_get_lcd_version(),
			    op_get_lcd_manufacture());
	push_component_info(BACKLIGHT, op_get_backlight_version(),
			    op_get_backlight_manufacture());
