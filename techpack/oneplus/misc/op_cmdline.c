...
static int __init boot_mode_init_normal(void)
{
	...
	char *substrnormal =
		strnstr(op_cmdline, "androidboot.mode=", strlen(op_cmdline));
...
{
	if (!cmdline_get_value(str, backlight_manufacture,
			       sizeof(backlight_manufacture))) {
		pr_err("%s failed, or param not found\n", __func__);
		return 0;
	}
...
{
	if (!cmdline_get_value(str, backlight_version,
			       sizeof(backlight_version))) {
		pr_err("%s failed, or param not found\n", __func__);
		return 0;
	}
